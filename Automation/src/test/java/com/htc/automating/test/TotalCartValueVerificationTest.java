...
public class TotalCartValueVerificationTest extends BaseTest{
	
	@Test(dataProvider = "madison_form_data",dataProviderClass = com.htc.automating.base.BaseDataProvider.class)
	public void verifyTheTotalCostOfAllProductsInCart(Map<String,String> user)  {
		try {
			...
			child=parent.createNode(" Verified total cart value ");
		}catch(MyException e) {
			e.printStackTrace();
			Assert.fail();
		}
